...
def main():
    ...
    if not LEADERBOARD_URL:
        raise ValueError("LEADERBOARD_URL is missing. Check your .env file.")
    ...
    # P.show()
    if not os.path.exists("./../Leaderboard-Map/"):
        os.makedirs("./../Leaderboard-Map/")
    P.savefig(f"./../Leaderboard-Map/leaderboard_map_{datetime.datetime.now().strftime('%y%m%d')}.png",
    bbox_inches="tight", dpi=100)

    removeLeaderboardFile()
    

def removeLeaderboardFile():

    if os.path.isfile("./leaderboard.json"):
        os.remove("./leaderboard.json")
    
def getLatestLeaderboard():
    ...
    if response.status_code == 200:
        ...
        print(f'Leaderboard saved to ./leaderboard.json')
...
def getOptions():
    ...
    input_filenames = ['leaderboard.json']
    """
...
def plotInit(event):
    ...
    if OPTS["figure_title_override"] is not None:
        ...
        figure_title = f"AoC Leaderboard {event}"
